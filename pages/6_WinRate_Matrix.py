...
<style>
  /* ── 全頁字級放大（適合 50 歲以上閱讀）2026-09-15 ───────────────
     與 pages/1_Dashboard.py 同一套：根字級 16px → 20px，
     本頁所有 rem 尺寸一次放大 1.25 倍；下方再補 Streamlit 內建元件的個別字級。
     注意：矩陣本體畫在 components.html 的 iframe 裡，不吃這段 CSS，
     其字級在 build_matrix_html() 內另外設定。                          */
  html { font-size: 20px; }
  body, .stApp, [data-testid="stAppViewContainer"] { font-size: 1rem; line-height: 1.65; }
  [data-testid="stMarkdownContainer"] p  { font-size: 1rem; line-height: 1.65; }
  ...
  [data-baseweb="select"] div { font-size: 1rem; }
  /* 指數頁籤與 3個月／1個月 子頁籤 */
  [data-baseweb="tab"] p, [data-baseweb="tab"] div { font-size: 1.02rem !important; }
  [data-baseweb="tab"] { padding-left: 14px; padding-right: 14px; }
  /* 勝率／下跌深度 radio */
  [data-testid="stRadio"] label p { font-size: 1.02rem !important; }
...
# ── Matrix HTML builder ────────────────────────────────────────────────
# 字級放大版（2026-09-15）：iframe 內獨立設 html{font-size:20px}，
# 並把格子字級、欄寬、列高一起加大，與頁面其他部分同一個視覺尺度。
ROW_H = {"wr": 104, "mae": 128}
CELL_W = 140      # 每個乖離欄最小寬度
STICKY_W = 116    # 左側 VIX 欄寬
...
def build_matrix_html(data_list, curr_dev_bin, curr_vix_bin, mode="wr"):
    ...
    legend_html = "".join(
        ...
        f'font-size:0.85rem;font-weight:600;border:1px solid {fg}22">{lbl}</span>'
        for lbl, bg, fg in legend_items
    )

    th = ("padding:10px 12px;background:#0f172a;border:1px solid #1e293b;"
          "white-space:nowrap;font-size:0.88rem;letter-spacing:.04em;text-transform:uppercase")
    header = (f'<th style="{th};text-align:left;min-width:{STICKY_W}px;color:#334155;'
    ...
    for d in present_devs:
        ...
        mk = ' <span style="color:#f59e0b;font-size:0.9rem">◀</span>' if is_curr else ""
        header += (f'<th style="{th};text-align:center;min-width:{CELL_W}px;color:{c};font-weight:{fw}">'
                   f'{d}{mk}</th>')
    ...
    for vix_b in present_vix:
        ...
        vix_td = (f'<td style="padding:10px 12px;background:#0f172a;border:1px solid #1e293b;'
                  f'color:{vc};font-weight:{vfw};font-size:0.9rem;white-space:nowrap;'
        ...
        for dev_b in present_devs:
            ...
            if key in lookup:
                ...
                if r["n"] < 5:
                    inner = '<span style="color:#374151;font-size:1.2rem">—</span>'
                elif mode == "wr":
                    win_s  = f'+{r["avg_win"]:.1f}%'
                    loss_s = f'{r["avg_loss"]:.1f}%'
                    sig_badge = ('<span style="color:#f59e0b;font-size:0.8rem;'
                                 'position:absolute;top:4px;right:6px">★</span>'
                                 if r.get("sig") else "")
                    inner = (
                        f'<div style="font-size:1.45rem;font-weight:800;line-height:1.2">{r["wr"]:.0f}%</div>'
                        f'<div style="font-size:0.85rem;margin-top:3px;opacity:0.95">'
                        f'<span style="color:#4ade80">↑{win_s}</span>'
                        f'<span style="color:#94a3b8;margin:0 3px">/</span>'
                        f'<span style="color:#f87171">↓{loss_s}</span>'
                        f'</div>'
                        f'<div style="font-size:0.78rem;opacity:0.5;margin-top:2px">n={r["n"]}</div>'
                        f'{sig_badge}'
                    )
                else:
                    inner = (
                        f'<div style="font-size:1.4rem;font-weight:800;line-height:1.2">{r["mae_med"]:.1f}%</div>'
                        f'<div style="font-size:0.85rem;margin-top:3px;opacity:0.9">'
                        f'均 {r["mae_avg"]:.1f} ｜ P95 {r["mae_p95"]:.1f}</div>'
                        f'<div style="font-size:0.82rem;margin-top:2px;opacity:0.75">'
            ...
            row_cells += (
                f'<td style="padding:10px 12px;text-align:center;background:{bg};'
                f'{border};{shadow};min-width:{CELL_W}px;position:relative">'
                f'<div style="color:{fg}">{inner}</div></td>'
            )
        rows += f"<tr>{row_cells}</tr>"

    table_h = 100 + len(present_vix) * ROW_H[mode]

    return f"""<!DOCTYPE html><html lang="zh-TW"><head>
<meta charset="UTF-8">
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
/* iframe 內獨立根字級：與主頁面同尺度（16px → 20px） */
html{{font-size:20px}}
body{{background:#0a0f1a;color:#e2e8f0;font-size:1rem;
      font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif}}
.legend{{display:flex;flex-wrap:wrap;gap:6px;padding:8px 0 12px;align-items:center}}
.legend-lbl{{font-size:0.85rem;color:#475569;margin-right:6px}}
...
<div class="legend">
  ...
  <span style="margin-left:10px;font-size:0.82rem;color:#475569">{legend_note}</span>
</div>
...
try:
    ...
    st.markdown(
        ...
        f"padding:6px 14px;margin-bottom:8px;font-size:0.9rem;color:#475569'>"
    ...
    BEAR_WARN = """<div style="background:#1c0a00;border:1px solid #854d0e;border-radius:8px;
                   padding:10px 16px;color:#fde68a;font-size:0.95rem;margin-bottom:10px">
    ...
    MAE_INFO = """<div style="background:#0f2027;border:1px solid #1e3a5f;border-radius:8px;
                  padding:10px 16px;color:#94a3b8;font-size:0.95rem;margin-bottom:10px;line-height:1.75">
    ...
    def _matrix_height(key, ticker, mode):
        ...
        return 100 + len(pv) * (ROW_H[mode] + 1)
        ...
        with tab_ui:
            ...
            with c1:
                st.markdown(f"""<div class="metric-card">
                  <div style="font-size:0.9rem;color:#64748b">{ticker} 最新收盤</div>
                  <div style="font-size:1.55rem;font-weight:700;color:#e2e8f0;margin:3px 0">${live['price']:,.2f}</div>
                  <div style="font-size:0.85rem;color:#475569">截至 {live['as_of']}</div>
                </div>""", unsafe_allow_html=True)
            with c2:
                st.markdown(f"""<div class="metric-card">
                  <div style="font-size:0.9rem;color:#64748b">EMA60 季線</div>
                  <div style="font-size:1.55rem;font-weight:700;color:#e2e8f0;margin:3px 0">${live['ema60']:,.2f}</div>
                </div>""", unsafe_allow_html=True)
            with c3:
                st.markdown(f"""<div class="metric-card">
                  <div style="font-size:0.9rem;color:#64748b">乖離率（EMA60）</div>
                  <div style="font-size:1.55rem;font-weight:700;color:{dev_color};margin:3px 0">{'+' if live['dev']>=0 else ''}{live['dev']:.2f}%</div>
                  <div style="font-size:0.85rem;color:#475569">桶：<b style="color:#f59e0b">{dev_bin}</b></div>
                </div>""", unsafe_allow_html=True)
            with c4:
                st.markdown(f"""<div class="metric-card">
                  <div style="font-size:0.9rem;color:#64748b">VIX 恐慌指數</div>
                  <div style="font-size:1.55rem;font-weight:700;color:{vix_color};margin:3px 0">{vix_v:.2f}</div>
                  <div style="font-size:0.85rem;color:#475569">桶：<b style="color:#f59e0b">{vix_bin}</b></div>
                </div>""", unsafe_allow_html=True)
            ...
            with c5:
                st.markdown(f"""<div class="metric-card">
                  <div style="font-size:0.9rem;color:#64748b">EMA260 年線狀態</div>
                  <div style="font-size:1.25rem;font-weight:700;color:{ma_color};margin:5px 0">{ma_text}</div>
                  <div style="font-size:0.85rem;color:#475569">${live['ema260']:,.2f}（{'+' if ema260_dev>=0 else ''}{ema260_dev:.2f}%）</div>
                </div>""", unsafe_allow_html=True)
            ...
            if cur_month_info:
                ...
                m_badge = (f'<span style="background:#1e3a5f;border:1px solid #334155;'
                           f'border-radius:6px;padding:3px 10px;font-size:0.9rem;color:#94a3b8">'
                           f'📅 {MONTH_ZH[cur_month]}歷史勝率（1m）：'
                           f'<b style="color:{m_color}">{m_wr:.1f}%</b>'
                           f'<span style="color:{m_color};font-size:0.85rem"> ({m_sign}{m_diff}% vs 全月均 {t_overall_wr:.1f}%)</span>'
            ...
            <div style="background:#1e293b;border:1px solid #334155;border-radius:10px;
                        padding:13px 20px;margin-bottom:8px;display:flex;align-items:center;gap:16px">
              <span style="font-size:1.8rem">{above_icon}</span>
              <div style="flex:1">
                <div style="font-size:1.02rem;color:#94a3b8">
                ...
                {'<div style="margin-top:8px">' + m_badge + '</div>' if m_badge else ''}
              </div>
            </div>""", unsafe_allow_html=True)
    ...
    # 「統計顯著性說明」兩張卡片已於 2026-09-15 移除（重疊樣本限制改列在下方統計說明）
    st.markdown("---")
    ...
    for col_ui, (title, color, line1, line2) in zip([cc1, cc2, cc3, cc4], comparisons):
        with col_ui:
            st.markdown(f"""<div class="insight-card">
              <div style="font-size:1.02rem;font-weight:700;color:{color};margin-bottom:8px">{title}</div>
              <div style="font-size:0.9rem;color:#94a3b8;line-height:1.75">{line1}<br>{line2}</div>
            </div>""", unsafe_allow_html=True)

    # ── Caveats ────────────────────────────────────────────────────────
    st.markdown(f"""
    <div style="background:#1c1400;border:1px solid #854d0e;border-radius:8px;
                padding:12px 18px;margin-top:4px;margin-bottom:6px;
                color:#fde68a;font-size:1rem;font-weight:600">
    ...
    <div style="background:#1e293b;border:1px solid #1e3a5f;border-radius:8px;
      ...
      <div style="font-size:0.9rem;color:#475569;line-height:1.85">
